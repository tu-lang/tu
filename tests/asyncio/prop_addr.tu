...
fn check_v4(a<u8>, b<u8>, c<u8>, d<u8>, port<u16>) {
    ...
    v4<net.SocketAddrV4> = net.SocketAddrV4::new(ip, port)
    ...
    if back.v4() == false os.die("v4 round-trip lost the v4 tag")

    a4<net.SocketAddrV4> = back
...
fn check_v6(seg<u16*>, port<u16>) {
    ...
    v6<net.SocketAddrV6> = net.SocketAddrV6::new(ip6, port, 0, 0)

    s<string.String> = anet.socket_addr_to_string(v6)
    err<i32>, back<net.SocketAddr> = anet.parse_socket_addr(s.str(), s.len())
    if err != io.Ok os.dief("v6 parse failed for round-trip")
    if back.v4() os.die("v6 round-trip gained a v4 tag")

    a6<net.SocketAddrV6> = back
